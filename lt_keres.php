...
class Lang{
 ...
 public static  $dirT=['app','mod','tmpl'];  // $lt_szovegnev='szöveg';
 public static $elemT=[];
 ...
 static public function  HTML_keres($file){
     ...
     {
         $rsT['value']=$elemek[4][$key] ?? '';
         $rsT['elem']=$elemek[1][$key] ?? '';
         $rsT['paramnev']=$elemek[2][$key] ?? '';
         $rsT['paramval']=$elemek[3][$key] ?? '';
